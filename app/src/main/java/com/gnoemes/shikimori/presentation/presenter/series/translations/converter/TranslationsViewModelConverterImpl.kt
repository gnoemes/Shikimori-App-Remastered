...
class TranslationsViewModelConverterImpl @Inject constructor(
    ...
    override fun convertTranslations(
        translations: List<Translation>,
        setting: TranslationSetting?
    ): List<TranslationViewModel> {


        return translations
            .mapIndexed { index, translation ->
                if (translation.author.isEmpty()) translation.copy(
                    author = "$unknownAuthor $index"
                ) else translation
            }
    ...
    private fun convertTranslation(
        it: Map.Entry<String, List<Translation>>,
        setting: TranslationSetting?
    ): TranslationViewModel? {
        if (it.value.isEmpty()) return null

        val author =
            if (it.key.contains(unknownAuthor)) it.key.replace(Regex("\\d"), "") else it.key
        ...
        val hasBd =
            it.value.find { it.quality == TranslationQuality.BD || it.quality == TranslationQuality.DVD } != null
    ...
    private fun convertVideo(t: Translation): TranslationVideo {
        ...
        val simpleAuthor =
            URLEncoder.encode(t.author.replace(Regex("\\(.+\\)"), "").trim(), "utf-8")
